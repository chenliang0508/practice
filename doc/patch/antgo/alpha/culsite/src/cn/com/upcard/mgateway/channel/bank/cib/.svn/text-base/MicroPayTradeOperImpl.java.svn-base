...
public class MicroPayTradeOperImpl extends AbstractUnifiedOper {
	...
	public ChannelResponseInfo cancel(ChannelRequestInfo channelRequestInfo) {
		ChannelResponseInfo channelRespInfo = new ChannelResponseInfo();
		//先判断原交易订单的时间
		Date tradeEndTime = channelRequestInfo.getTradeEndTime();
		//订单时间15s以后的时间
		Date after15s=DateUtil.afterSecond(tradeEndTime, 15);
		//点单时间4分30秒的时间
		Date after270s= DateUtil.afterSecond(tradeEndTime, 270);
		//当前时间
		Date now = DateUtil.now();
		logger.info("比较当前时间与订单时间的时间差");
		//如果当前时间在15秒之内的话 直接返回撤销失败
		if(now.before(after15s)) {
			channelRespInfo.setResponseResult(ResponseResult.CANCEL_TIME_SHORT);
			return channelRespInfo;
		}
		//如果时间在4分30秒以后调用退款接口
		if(now.after(after270s)) {
			...
			refundInfo.setTransactionId(channelRequestInfo.getTransactionId());
			refundInfo.setOutRefundNo(this.createTradeNo(BankCode.CIB));
			refundInfo.setTotalFee(channelRequestInfo.getTotalFee());
			refundInfo.setRefundFee(channelRequestInfo.getRefundFee());
			refundInfo.setOpUserId(channelRequestInfo.getMchId());
			refundInfo.setCommPrivateKey(channelRequestInfo.getCommPrivateKey());
			ChannelResponseInfo refundResponse = super.refund(refundInfo);
