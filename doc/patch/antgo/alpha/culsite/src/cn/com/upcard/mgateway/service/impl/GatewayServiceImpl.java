...
public class GatewayServiceImpl implements GatewayService {
	...
	public AlipayNativeResponse alipayNativePreOrder(PreTradeOrderInfo preTradeOrderInfo) {
		...
		// 调用第三方预下单
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(TradeType.ALI_NATIVE.name(), preTradeOrderInfo.getBankCode()).pay(channelRequestInfo);
	...
	public AlipayJsapiResponse alipayJsapiPreOrder(PreTradeOrderInfo preTradeOrderInfo) {
		...
		OrderOper orderOper = orderOperService.saveOrderOper(order, OrderOperType.PAY.name(), null, preTradeOrderInfo.getThirdBuyerId(), preTradeOrderInfo.getReqIp());
		// 更新订单和订单操作
		Date now = DateUtil.now();
		order.setStatus(OrderStatus.WAITING.name()).setUpdateTime(now);
		orderOperService.updateTradeOrder(order);
		...
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(TradeType.ALI_JSAPI.name(), preTradeOrderInfo.getBankCode()).pay(channelRequestInfo);
		if (channelReponseInfo == null) {
			logger.info("调用第三方接口无应答");
			orderOper.setStatus(OrderOperStatus.NON_RESP.name()).setUpdateTime(DateUtil.now());
			orderOperService.updateOrderOper(orderOper);
	...
	public WxNativeResponse wxNativePreOrder(PreTradeOrderInfo preTradeOrderInfo) {
		...
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(TradeType.WX_NATIVE.name(), preTradeOrderInfo.getBankCode()).pay(requestInfo);
		if (channelReponseInfo == null) {
			logger.info("调用第三方接口无应答");
			resp.setCode(ResponseResult.NON_RESP.getCode());
			resp.setMsg(ResponseResult.NON_RESP.getMsg());
	...
	public WxJsResponse wxJsPreOrder(PreTradeOrderInfo preTradeOrderInfo) {
		...
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(TradeType.WX_JSAPI.name(), preTradeOrderInfo.getBankCode()).pay(requestInfo);
		if (channelReponseInfo == null) {
			logger.info("调用第三方接口无应答");
			resp.setCode(ResponseResult.NON_RESP.getCode());
			resp.setMsg(ResponseResult.NON_RESP.getMsg());
	...
	public TradeOrder createTradeOrder(PreTradeOrderInfo preTradeOrderInfo) {
		// 判断订单号是否重复
		TradeOrder order = orderOperService.findByOrderNoAndCulCommNoAndType(preTradeOrderInfo.getOutTradeNo(), preTradeOrderInfo.getCommUnionNo(), null);
		if (order != null) {
	...
	public TradeQueryResponse unifiedPayQuery(PreTradeOrderInfo preTradeOrderInfo) {
		...
		if (StringUtils.isNotEmpty(tradeNo)) {
			tradeOrder = orderOperService.findByTradeNoAndOrderNoAndCulCommNoAndType(tradeNo, orderNo, culCommNo, OrderType.PAY.name());
		} else {
		...
		if (OrderStatus.WAITING.name().equals(tradeOrder.getStatus())) {
			...
			ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(tradeOrder.getTradeType(), preTradeOrderInfo.getBankCode()).tradeQuery(channelRequestInfo);
	...
	public MicroPayResponse microPayOrder(PreTradeOrderInfo preTradeOrderInfo) {
		...
		// 调用支付接口
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(preTradeOrderInfo.getTradeType(), preTradeOrderInfo.getBankCode()).pay(channelRequestInfo);
	...
	public TradeCancelResponse tradeCancel(PreTradeOrderInfo preTradeOrderInfo) {
		...
		// 支付交易查询
		if (StringUtils.isNotEmpty(tradeNo)) {
			order = orderOperService.findByTradeNoAndOrderNoAndCulCommNoAndType(tradeNo, orderNo, culCommNo, OrderType.PAY.name());
		} else {
		...
		if (!(TradeType.ALI_DIRECT.name().equals(order.getTradeType()) || TradeType.WX_MICROPAY.name().equals(order.getTradeType()))) {
		...
		// 这里添加撤销金额，（民生银行没有撤销接口，撤销为退款接口包装，故添加全部退款金额）
		channelRequestInfo.setTradeEndTime(tradeEndTime);
		channelRequestInfo.setTotalFee(String.valueOf(order.getOrderPoint()));
		channelRequestInfo.setRefundFee(String.valueOf(order.getOrderPoint()));
		channelRequestInfo.setMchId(preTradeOrderInfo.getBankMerNo());
		channelRequestInfo.setOutTradeNo(order.getTradeNo());
		channelRequestInfo.setCommPrivateKey(preTradeOrderInfo.getCommPrivateKey());
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(order.getTradeType(), preTradeOrderInfo.getBankCode()).cancel(channelRequestInfo);
		if (channelReponseInfo == null) {
			logger.info("无返回信息");
			orderOper.setStatus(OrderOperStatus.NON_RESP.name()).setUpdateTime(DateUtil.now());
			orderOperService.updateOrderOper(orderOper);
	...
	public TradeRefundResponse unifiedRefund(PreTradeOrderInfo preTradeOrderInfo) {
		...
		logger.info("orderNo:" + orderNo + ",tradeNo:" + tradeNo + ",outRefundNo:" + outRefundNo + ",culCommNo:" + culCommNo);
		TradeOrder tradeOrder = null;
		if (StringUtils.isNotEmpty(tradeNo)) {
			tradeOrder = orderOperService.findByTradeNoAndOrderNoAndCulCommNoAndType(tradeNo, orderNo, culCommNo, OrderType.PAY.name());
		} else {
		...
		if (OrderStatus.INIT.name().equals(tradeOrder.getStatus()) || OrderStatus.WAITING.name().equals(tradeOrder.getStatus())) {
		...
		List<TradeOrder> refundOrders = orderOperService.findByOrigOrderNoAndCulCommNoAndStatus(tradeOrder.getOrderNo(), culCommNo, refundOrderStatus);
		...
		OrderOper refundOrderOper = orderOperService.saveOrderOper(refundOrder, OrderOperType.REFUND.name(), null, null, null);
		// 更新订单和订单操作
		Date now = DateUtil.now();
		...
		channelRequestInfo.setOutTradeNo(tradeOrder.getTradeNo());
		if (StringUtils.isNotBlank(preTradeOrderInfo.getOperUser())) {
			channelRequestInfo.setOpUserId(preTradeOrderInfo.getOperUser());
		} else {
			channelRequestInfo.setOpUserId(preTradeOrderInfo.getBankMerNo());
		}
		...
		channelRequestInfo.setTransactionId(tradeOrder.getChannelTxnNo());
		channelRequestInfo.setCommPrivateKey(preTradeOrderInfo.getCommPrivateKey());
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(tradeOrder.getTradeType(), preTradeOrderInfo.getBankCode()).refund(channelRequestInfo);
	...
	public RefundQueryResponse tradeRefundQuery(PreTradeOrderInfo preTradeOrderInfo) {
		...
		if (StringUtils.isNotEmpty(refundTradeNo)) {
			tradeOrder = orderOperService.findByTradeNoAndOrderNoAndCulCommNoAndType(refundTradeNo, outRefundNo, culCommNo, OrderType.REFUND.name());
		} else {
		...
		if (OrderStatus.SUCCESS.name().equals(tradeOrder.getStatus())) {
			// 退款成功
			resp.setMchId(tradeOrder.getCulCommNo()).setOutTradeNo(tradeOrder.getOrigOrderNo()).setRefundNo(tradeOrder.getOrderNo()).setRefundId(tradeOrder.getTradeNo())
					.setRefundAmount(String.valueOf(tradeOrder.getOrderPoint())).setTradeState(OrderStatus.SUCCESS.name())
		...
		if (OrderStatus.WAITING.name().equals(tradeOrder.getStatus())) {
			// 民生银行的退款查询需要原交易订单号
			TradeOrder order = orderOperService.findByOrderNoAndCulCommNoAndType(tradeOrder.getOrderNo(), culCommNo, OrderType.PAY.name());
			if (order == null) {
				logger.info("未查询到退款订单信息");
				resp.setCode(ResponseResult.ORDER_NOT_EXISTS.getCode());
				resp.setMsg(ResponseResult.ORDER_NOT_EXISTS.getMsg());
			...
			channelRequestInfo.setCommPrivateKey(preTradeOrderInfo.getCommPrivateKey());
			ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(tradeOrder.getTradeType(), preTradeOrderInfo.getBankCode()).refundQuery(channelRequestInfo);
	...
	public TradecloseResponse unifiedClose(PreTradeOrderInfo preTradeOrderInfo) {
		...
		channelRequestInfo.setOutTradeNo(order.getTradeNo());
		channelRequestInfo.setCommPrivateKey(preTradeOrderInfo.getCommPrivateKey());
		ChannelResponseInfo channelReponseInfo = ThirdChannelTradeFactory.produce(order.getTradeType(), preTradeOrderInfo.getBankCode()).close(channelRequestInfo);
