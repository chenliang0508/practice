...
public class CallbackController extends BaseController {
	...
	private static final String CMBC_RETURN_SUCCESS = "S";
	...
	public String cibNotify(HttpServletRequest request) {
		...
		try {
			String resquestStr = XmlUtils.parseRequst(request);
			logger.info("resquestStr:{}", resquestStr);
			if (StringUtils.isEmpty(resquestStr)) {
				return FAIL;
			}
			resultMap = XmlUtils.toMap(resquestStr.getBytes(), "utf-8");
	...
	@ResponseBody
	@RequestMapping(method = { RequestMethod.POST, RequestMethod.GET }, value = "cmbc")
	public String cmbcNotify(HttpServletRequest request) {
		...
		String dncryptContext = SignatureUtils.dncrypt(context);

		if (!SUCCESS.equals(SignatureUtils.signCheck(dncryptContext))) {
			logger.info("Signature verification error");
			return FAIL;
		}

		Map<String, String> dataMap = JsonUtils.jsonToMap(dncryptContext);
		String merchantNo = dataMap.get("merchantNo");// 商户号
		String orderNo = dataMap.get("orderNo");// 商户订单号 银商
		String platformId = dataMap.get("platformId");// 平台号
		String amount = dataMap.get("amount");// 交易金额
		String voucherNo = dataMap.get("voucherNo");// 凭证号
		String bankTradeNo = dataMap.get("bankTradeNo");// 收单系列流水号 民生
		String tradeStatus = dataMap.get("tradeStatus");// 交易结果
		String reserve = dataMap.get("reserve");// 保留域
		String refNo = dataMap.get("refNo");// 参考号
		String batchNo = dataMap.get("batchNo");// 批次号
		String cardType = dataMap.get("cardType");// 卡类型
		String cardNo = dataMap.get("cardNo");// 卡号
		String cbCode = dataMap.get("cbCode");// 发卡行行号
		String cardName = dataMap.get("cardName");// 发卡行行名
		String fee = dataMap.get("fee");// 交易手续费
		String cupTermId = dataMap.get("cupTermId");// 银联终端号
		String cupTsamNo = dataMap.get("cupTsamNo");// 设备序列号
		String centerInfo = dataMap.get("centerInfo");// 其他信息
		String centerSeqId = dataMap.get("centerSeqId");// 微信/支付宝订单号
		String serialNo = dataMap.get("serialNo");// 收单流水号
		String bankOrderNo = dataMap.get("bankOrderNo");// 收单到微信/支付宝下单编号
		// 解析其他信息获取里面的相关参数
		logger.info("开始解析centerInfo，centerInfo=" + centerInfo);
		Map<String, String> infoMap = CMBCToolUtils.analyzeCMBCMessageToMap(centerInfo);
		String timeEnd = infoMap.get("time_end");
		if (StringUtils.isBlank(timeEnd)) {
			timeEnd = DateUtil.format2(DateUtil.now(), DateUtil.yyyyMMddHHmmss);
		}
		TradeOrder tradeOrder = orderOperService.findOrderByTradeNo(orderNo, false);
		if (null == tradeOrder) {
			logger.error("交易号不存在，outTradeNo={}", orderNo);
			return FAIL;
		}
		// 交易类型没有返回只有到数据库中查找
		...
		CallbackRequestInfo callbackRequestInfo = new CallbackRequestInfo();
		callbackRequestInfo.setTradeType(tradeType);
		callbackRequestInfo.setTransactionId(bankTradeNo);
		callbackRequestInfo.setOutTransactionId(centerSeqId);
		callbackRequestInfo.setPayResult(CMBC_RETURN_SUCCESS.equals(tradeStatus) ? "0" : tradeStatus);
		callbackRequestInfo.setSubAppid(infoMap.get("app_id"));
		callbackRequestInfo.setSubOpenid(infoMap.get("openid"));
		callbackRequestInfo.setAliOpenid(infoMap.get("buyer_id"));
		callbackRequestInfo.setOutTradeNo(orderNo);
		if (StringUtils.isNotBlank(amount)) {
			callbackRequestInfo.setTotalFee(Integer.valueOf(amount));
		}
		callbackRequestInfo.setTimeEnd(timeEnd);
		...
		TradeOrder trade = processingResult.getTradeOrder();
		if (null != trade) {
			apiLogService.saveApiLog(trade.getId(), OrderOperType.PAY, JSON.toJSONString(dataMap));
		}
		if (processingResult.isNeedPush()) {
			// 组装消息推送内容
			Map<String, String> contentMap = new HashMap<String, String>();
			contentMap.put("code", CMBC_RETURN_SUCCESS.equals(tradeStatus) ? "00" : "71"); // 支付结果
			contentMap.put("msg", CMBC_RETURN_SUCCESS.equals(tradeStatus) ? "success" : "fail"); // 结果描述
			contentMap.put("mchId", trade.getCulCommNo()); // 商户号
			// contentMap.put("deviceInfo", deviceInfo); // 设备号
			contentMap.put("tradeType", tradeType); // 交易类型
			contentMap.put("outTradeNo", trade.getOrderNo()); // 商户订单号
			contentMap.put("transactionId", trade.getTradeNo()); // 银商订单号
			contentMap.put("subAppid", infoMap.get("app_id")); // 商户appid
			contentMap.put("buyerAccount", processingResult.getChannelResponseInfo().getThirdBuyerAccount()); // 买家账号
			// contentMap.put("subIsSubscribe", subIsSubscribe); // 是否关注公众号
			// contentMap.put("feeType", feeType); // 货币类型
			contentMap.put("attach", reserve); // 附加信息
			if (StringUtils.isNotBlank(amount)) {
				contentMap.put("totalFee", String.valueOf(amount));// 订单金额
			}
			// contentMap.put("payFee", String.valueOf(payFee)); // 支付金额
			// contentMap.put("discountFee", String.valueOf(discountFee)); // 折扣金额
			contentMap.put("timeEnd", timeEnd); // 交易完成时间
			CallbackPara callbackPara = new CallbackPara();
			callbackPara.setOrderId(trade.getId());
			callbackPara.setUrl(trade.getNotifyUrl());
			callbackPara.setContentMap(contentMap);
			// 推送消息
			callbackService.callbackHandle(callbackPara);
		}
	...
	public String lklNotify(HttpServletRequest request) {
		...
		String[] signField = { "responseCode", "compOrgCode", "reqLogNo", "ornReqLogNo", "reqTm", "payChlTyp", "mercId", "txnAmt", "txnTm", "tradeType", "sub_appid", "payOrderId",
				"merOrderNo" };
		// 验证签名
